...
public class RecentBroadcastBusiness {
  ...
  private final Map<BusinessAction, Consumer<List<Broadcast>>> strategies;
  ...
  public void handleRecentBroadcasts(BusinessAction action, Channel channel) {
    try {
      ...
      List<Broadcast> recentBroadcasts = broadcastRepository.getRecentBroadcasts(channel);
      strategies.get(action).accept(recentBroadcasts);
    // TODO Clarify which exceptions the lib will throw
    } catch (IOException e) {
  ...
  private Consumer<List<Broadcast>> saveRecentBroadcasts() {
    return broadcastRepository::updateBroadcasts;
  }

  private Consumer<List<Broadcast>> displayRecentBroadcasts() {
