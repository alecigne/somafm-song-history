...
class SomaFmSongHistoryRestIT {
  ...
  @BeforeEach
  void setUp() {
    JavalinRestController controller = JavalinRestController.init(getBroadcastsUseCase, getSongsUseCase, fetchRecentBroadcastsUseCase);
  ...
  void should_get_broadcasts() {
    ...
    response.then()
        ...
        .body("items[0].song.artist", equalTo(dirkSerriesSix().artist().name()))
  ...
  void should_get_songs() {
    // Given
    getSongsUseCase.page = new Page<>(2, 2, 5, 3, List.of(dirkSerriesSix(), igneousFlameIncandescentArc()));
    RequestSpecification req = given().port(port);
    ...
    response.then()
        ...
        .body("totalPages", equalTo(3))
        .body("items", hasSize(2))
        .body("items[0].artist", equalTo(dirkSerriesSix().artist().name()))
        .body("items[0].title", equalTo(dirkSerriesSix().title()))
        .body("items[0].album", equalTo(dirkSerriesSix().album()))
        .body("items[1].artist", equalTo(igneousFlameIncandescentArc().artist().name()))
  ...
  void should_get_recent_broadcasts_for_channel() {
    ...
    response.then()
        ...
        .body("[0].song.artist", equalTo(dirkSerriesSix().artist().name()))
