...
class CLITest {
  ...
  void should_log_error_if_wrong_number_of_args() {
    ...
    var expected = "You must enter 2 arguments: action and channel.";
  ...
  static Stream<Arguments> should_run_command() {
    return Stream.of(
        argumentSet("With display action", "display"),
        argumentSet("With unknown action", "unknown_action")
    );
  }
